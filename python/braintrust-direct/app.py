"""
Braintrust + Last9 dual-export example — direct mode.

Two SpanProcessors on a single TracerProvider:
- BraintrustSpanProcessor (from `braintrust[otel]`) routes spans to Braintrust
  via the x-bt-parent header.
- BatchSpanProcessor(OTLPSpanExporter) ships the same spans to Last9.

Both backends receive identical traces with matching trace/span IDs.
"""
...
resource = Resource.create({
    SERVICE_NAME: os.environ.get("OTEL_SERVICE_NAME", "braintrust-direct-example"),
...
last9_endpoint = os.environ["LAST9_OTLP_ENDPOINT"]
last9_auth = os.environ["LAST9_OTLP_AUTH"]
provider.add_span_processor(BatchSpanProcessor(OTLPSpanExporter(
    endpoint=f"{last9_endpoint.rstrip('/')}/v1/traces",
    headers={"Authorization": last9_auth},
)))

trace.set_tracer_provider(provider)
tracer = trace.get_tracer("braintrust-direct-example")
client = OpenAI()
