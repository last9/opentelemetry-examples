        ...
        return function wrapQuery(original) {
            return function wrapped_logging_method(query) {
                ...
                const attributes = {
                    ...
                    [semantic_conventions_1.SEMATTRS_NET_TRANSPORT]: config?.connection?.filename === ':memory:' ? 'inproc' : undefined,
                };
                if (maxQueryLength) {
                    // filters both undefined and 0
                    attributes[semantic_conventions_1.SEMATTRS_DB_STATEMENT] = utils.limitLength(query?.sql, maxQueryLength);
                }
